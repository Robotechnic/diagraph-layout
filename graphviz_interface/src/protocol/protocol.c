...
int decode_Node(uint8_t *__input_buffer, size_t buffer_len, Node *out, size_t *buffer_offset) {
    ...
    if (has_xlabel) {
        ...
        if ((err = decode_Size(__input_buffer + __buffer_offset, buffer_len - __buffer_offset,
                               &out->xlabel[0], &__buffer_offset))) {
            return err;
        }
    } else {
        out->xlabel = NULL;
...
int decode_Edge(uint8_t *__input_buffer, size_t buffer_len, Edge *out, size_t *buffer_offset) {
    ...
    if (out->attributes_len == 0) {
        ...
        for (size_t i = 0; i < out->attributes_len; i++) {
            if ((err = decode_Attribute(__input_buffer + __buffer_offset, buffer_len - __buffer_offset,
                                        &out->attributes[i], &__buffer_offset))) {
                return err;
            }
        }
    }
    ...
    if (has_label) {
        ...
        if ((err = decode_Size(__input_buffer + __buffer_offset, buffer_len - __buffer_offset, &out->label[0],
                               &__buffer_offset))) {
            return err;
        }
    } else {
    ...
    if (has_headlabel) {
        ...
        if ((err = decode_Size(__input_buffer + __buffer_offset, buffer_len - __buffer_offset,
                               &out->headlabel[0], &__buffer_offset))) {
            return err;
        }
    } else {
        out->headlabel = NULL;
    ...
    if (has_taillabel) {
        ...
        if ((err = decode_Size(__input_buffer + __buffer_offset, buffer_len - __buffer_offset,
                               &out->taillabel[0], &__buffer_offset))) {
            return err;
        }
    } else {
        out->taillabel = NULL;
...
int decode_GraphAttribute(uint8_t *__input_buffer, size_t buffer_len, GraphAttribute *out,
...
int encode_LayoutLabel(const LayoutLabel *s, uint8_t *__input_buffer, size_t *buffer_len,
                       size_t *buffer_offset) {
    size_t __buffer_offset = 0;
    size_t s_size = LayoutLabel_size(s);
...
size_t LayoutNode_size(const void *s) {
    return string_size(((LayoutNode *)s)->name) + TYPST_INT_SIZE + TYPST_INT_SIZE + TYPST_INT_SIZE +
           TYPST_INT_SIZE + optional_size(((LayoutNode *)s)->xlabel, LayoutLabel_size);
}
int encode_LayoutNode(const LayoutNode *s, uint8_t *__input_buffer, size_t *buffer_len,
                      size_t *buffer_offset) {
    size_t __buffer_offset = 0;
    size_t s_size = LayoutNode_size(s);
    if (s_size > *buffer_len) {
        return 2;
    }
    int err;
    ...
    if (s->xlabel) {
        if ((err = encode_LayoutLabel(&s->xlabel[0], __input_buffer + __buffer_offset, buffer_len,
                                      &__buffer_offset))) {
            return err;
        }
    }
...
int encode_ControlPoint(const ControlPoint *s, uint8_t *__input_buffer, size_t *buffer_len,
                        size_t *buffer_offset) {
    size_t __buffer_offset = 0;
    size_t s_size = ControlPoint_size(s);
    if (s_size > *buffer_len) {
        return 2;
    }
    int err;
...
size_t LayoutEdge_size(const void *s) {
    return TYPST_INT_SIZE +
           list_size(((LayoutEdge *)s)->points, ((LayoutEdge *)s)->points_len, ControlPoint_size,
                     sizeof(*((LayoutEdge *)s)->points)) +
           string_size(((LayoutEdge *)s)->head) + string_size(((LayoutEdge *)s)->tail) +
           optional_size(((LayoutEdge *)s)->label, LayoutLabel_size) +
           optional_size(((LayoutEdge *)s)->xlabel, LayoutLabel_size) +
           optional_size(((LayoutEdge *)s)->headlabel, LayoutLabel_size) +
           optional_size(((LayoutEdge *)s)->taillabel, LayoutLabel_size);
}
int encode_LayoutEdge(const LayoutEdge *s, uint8_t *__input_buffer, size_t *buffer_len,
                      size_t *buffer_offset) {
    size_t __buffer_offset = 0;
    size_t s_size = LayoutEdge_size(s);
    if (s_size > *buffer_len) {
        return 2;
    }
    int err;
    ...
    for (size_t i = 0; i < s->points_len; i++) {
        if ((err = encode_ControlPoint(&s->points[i], __input_buffer + __buffer_offset, buffer_len,
                                       &__buffer_offset))) {
            return err;
        }
    }
    ...
    if (s->label) {
        if ((err = encode_LayoutLabel(&s->label[0], __input_buffer + __buffer_offset, buffer_len,
                                      &__buffer_offset))) {
            return err;
        }
    }
    ...
    if (s->headlabel) {
        if ((err = encode_LayoutLabel(&s->headlabel[0], __input_buffer + __buffer_offset, buffer_len,
                                      &__buffer_offset))) {
            return err;
        }
    }
    ...
    if (s->taillabel) {
        if ((err = encode_LayoutLabel(&s->taillabel[0], __input_buffer + __buffer_offset, buffer_len,
                                      &__buffer_offset))) {
            return err;
        }
    }
...
size_t Layout_size(const void *s) {
    return 1 + TYPST_INT_SIZE + TYPST_INT_SIZE + TYPST_INT_SIZE + TYPST_INT_SIZE +
           list_size(((Layout *)s)->nodes, ((Layout *)s)->nodes_len, LayoutNode_size,
                     sizeof(*((Layout *)s)->nodes)) +
           TYPST_INT_SIZE +
           list_size(((Layout *)s)->edges, ((Layout *)s)->edges_len, LayoutEdge_size,
                     sizeof(*((Layout *)s)->edges));
}
int encode_Layout(const Layout *s) {
    ...
    for (size_t i = 0; i < s->nodes_len; i++) {
        if ((err = encode_LayoutNode(&s->nodes[i], __input_buffer + __buffer_offset, &buffer_len,
                                     &__buffer_offset))) {
            return err;
        }
    }
    ...
    for (size_t i = 0; i < s->edges_len; i++) {
        if ((err = encode_LayoutEdge(&s->edges[i], __input_buffer + __buffer_offset, &buffer_len,
                                     &__buffer_offset))) {
            return err;
        }
    }
...
int decode_Graph(size_t buffer_len, Graph *out) {
    ...
    if (out->edges_len == 0) {
        ...
        for (size_t i = 0; i < out->edges_len; i++) {
            if ((err = decode_Edge(__input_buffer + __buffer_offset, buffer_len - __buffer_offset,
                                   &out->edges[i], &__buffer_offset))) {
                return err;
            }
        }
    }
    ...
    if (out->nodes_len == 0) {
        ...
        for (size_t i = 0; i < out->nodes_len; i++) {
            if ((err = decode_Node(__input_buffer + __buffer_offset, buffer_len - __buffer_offset,
                                   &out->nodes[i], &__buffer_offset))) {
                return err;
            }
        }
    }
    ...
    if (out->attributes_len == 0) {
        ...
        for (size_t i = 0; i < out->attributes_len; i++) {
            if ((err = decode_GraphAttribute(__input_buffer + __buffer_offset, buffer_len - __buffer_offset,
                                             &out->attributes[i], &__buffer_offset))) {
                return err;
            }
        }
